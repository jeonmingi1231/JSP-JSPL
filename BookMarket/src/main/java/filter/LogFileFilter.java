...
public class LogFileFilter implements Filter { //jakarta.servlet.Filter
   ...
   public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
      ...
      writer.println("접근한 URL 경로 (custom) : "+ getURLPath(request));
      writer.println("요청 처리 시작 시간 (custom) :" + getCurrentTime());
      chain.doFilter(request, response); // 필터에서 요청과 응답에 대한 자료를 전송함.
      
      long end = System.currentTimeMillis();//종료시간 저장용
      writer.println("요청 처리 종료 시간 (custom) : "+getCurrentTime());
      writer.println("요청 처리 소요 시간 (시작 - 종료) : "+ (end-start)+"ms");
